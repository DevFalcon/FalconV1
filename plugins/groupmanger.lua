...
local function modadd(msg)
    ...
    if not is_admin(msg) then
        return '*🚩| ٱنۧتِٰ ݪسِټ ﻣۘـطۅر آݪبـﯛټ*'
end
    local data = load_data(_config.moderation.data)
  if data[tostring(msg.to.id)] then
   return '*• ٱݪحمٱيـِﮥ ﺑـِٱݪفعݪ مۘفعݪهِ ۿٖنآ ✔️\n🆔| ٱݪمِطۄړ: ['..msg.from.id..']*'
end
...
user_name = check_markdown(v.user.first_name)
  ...
  save_data(_config.moderation.data, data)
      ...
      save_data(_config.moderation.data, data)
  return '*• ٺـِﻡۘ تفعّٖيݪ ٱﻟﺣِـِمۘآﭔﮤ ۿٖـڼآ ៲❘៲ 🚸️*\n*🆔| ٱݪمِطۄړ : ['..msg.from.id..']*'
...
local function modrem(msg)
    -- superuser and admins only (because sudo are always has privilege)
      if not is_admin(msg) then
        return '*🚩| ٱنۧتِٰ ݪسِټ ٱډﻣۘـن آݪبـﯛټ*'
   end
    local data = load_data(_config.moderation.data)
    local receiver = msg.to.id
  if not data[tostring(msg.to.id)] then
    return '*• ٱݪحمٱيـِﮥ ﺑـِٱݪفعݪ ﻣۗعّٖطݪه✖️*\n*🆔| ٱݪمِطۄړ : ['..msg.from.id..']*'
  end
  ...
  save_data(_config.moderation.data, data)
     local groups = 'groups'
      ...
      save_data(_config.moderation.data, data)
  return '*• ٺـِﻡۘ ټـعّٖطُيݪ ٱﻟﺣِـِمۘآﭔﮤ ۿٖـڼآ ៲❘៲ 🚸\n🆔| ٱݪمِطۄړ : ['..msg.from.id..']*'
end
...
local function ownerlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['owners']) == nil then --fix way
    return "*🚩| ـۧلاٰ ېۄجډ مۘډړا۽ ۿٖنآ*"
end
   message = '*🚩| ﻗـٱئمۘه ٱلمۘډړا۽ :*\n\n'
...
local function filter_word(msg, word)
    ...
    if data[tostring(msg.to.id)]['filterlist'][(word)] then
        return "*🚩| ٱݪڪِݪمۧهِ * *["..word.."]* *ﻣۘضٱفِهْ ٱݪۍ ٲݪمِمڼۄعّٖآټ*"
      end
    data[tostring(msg.to.id)]['filterlist'][(word)] = true
    save_data(_config.moderation.data, data)
      return "*🚩| ٱݪڪِݪمۧهِ* *["..word.."]* *ټم مڼعّٖۿٖآ*"
    end
...
local function unfilter_word(msg, word)
    ...
    if data[tostring(msg.to.id)]['filterlist'][word] then
      ...
      save_data(_config.moderation.data, data)
        return "*🚩| ٱݪڪِݪمۧهِ * *["..word.."]* *ټـم ٱزاݪټها*"
    else
        return "*🚩| ٱݪڪِݪمۧهِ * *["..word.."]* *ټـم ﺑـٱﻟفِعݪ ٱزاݪټها*"
    end
  end

local function lock_link(msg, data, target)
if not is_mod(msg) then
return "*🚩| لـۧاٰ ٺبـٖعّٖبصَ*"
end

local lock_link = data[tostring(target)]["settings"]["lock_link"] 
if lock_link == "yes" then
 return "*🚩| آݪرﯛابطِ مۘقٛفٛـٖﯛلٰهِ آخِـٖﻱ*"
else
data[tostring(target)]["settings"]["lock_link"] = "yes"
save_data(_config.moderation.data, data) 
 return "*🚩| تٛـٖمٰ قٛفٛـٖلٰ آݪرﯛابطِ*\n*🚩| اٛيٓدٛيٖڱ ⌁ *"..msg.from.id..""
end
end
...
return "*🚩| آݪرﯛابطِ مۘفٛتٛـٖﯛحٰهَ آخِـٖﻱ*"
else 
data[tostring(target)]["settings"]["lock_link"] = "no" 
save_data(_config.moderation.data, data) 
return "*🚩| تٛـٖمٰ فٛتٛـٖحٰ آݪرﯛابطِ*\n*🚩| اٛيٓدٛيٖڱ ⌁ *"..msg.from.id..""
end
end
...
if lock_tag == "yes" then
 return "*🚩| آݪټٱك مۘقٛفٛـٖﯛلٰهِ آخِـٖﻱ*"
else
...
return "*🚩| ټـم آݪغـآ۽ ټثبێٺۿٖآ*"
elseif not is_owner(msg) then
   return 
 end
 elseif lock_pin == 'no' then
unpinChatMessage(msg.to.id)
return "*🚩| ټـم آݪغـآ۽ ټثبێٺۿٖآ️*"
end
end
...
return group_settings(msg, target)
   ...
   if data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] then
    return "*👁‍🗨| ﺑـٱلفعّٖـِݪ ټـم ړفعّٖـﮥ ﻣۗډيـُړ*\n*👁‍🗨| معّٖرفـُﮥ : *`[`"..username.."`]`"
    else
  data[tostring(msg.to.id)]['owners'][tostring(msg.reply.id)] = username
    save_data(_config.moderation.data, data)
    return "*👁‍🗨| ټـم ړفعّٖـﮥ ﻣۗډيـُړ*\n*👁‍🗨| معّٖرفـُﮥ : *`[`"..username.."`]`"
      end
